...
def test_query(query: str, query_number: int):
    """
    Test a single query through the full pipeline
    
    Args:
        query: Query string to test
        query_number: Number of the query (1-4)
    """
    print(f"\n{'='*80}")
    print(f"Test {query_number}: Factual Query")
    print(f"Query: {query}")
    print(f"{'='*80}")
